...
const ImmersiveOverlay: React.FC<ImmersiveOverlayProps> = ({ language }) => {
    ...
    useEffect(() => {
        ...
        const animate = () => {
            ...
            // Check if idle (>3s)
            const now = Date.now();
            const isIdle = now - lastInteraction.current > 3000;

            // Only spawn particles if mouse is on screen and valid and NOT idle
            if (!isIdle && mouseRef.current.x > 0 && mouseRef.current.y > 0) {
    ...
    useEffect(() => {
        ...
        }, 90000); // Every 90 seconds (1.5 minutes) - reduced frequency
    ...
    return (
        <>
            ...
            <canvas ref={canvasRef} className="fixed inset-0 pointer-events-none z-[70] particle-trail" />
